...
export default function App() {
  ...
  const handleExport = (draft) => {
    ...
    const automationCharts = () => {
      ...
      const phaseCard = (phaseNum, phase, color, bgColor, title, subtitle) => {
        if (!phase) return "";
        const pct = phase.pct || 0;
        const phaseKey = `phase${phaseNum}`;
        const phaseTopics = topics?.[phaseKey] || [];

        return `
          <div style="flex:1;background:${bgColor};border:1.5px solid ${color};border-radius:12px;padding:20px;min-width:280px">
            <div style="font-size:15px;font-weight:700;color:${ZD.licorice};margin-bottom:3px">Phase ${phaseNum}: ${title}</div>
            <div style="font-size:11px;color:rgba(0,0,0,0.5);margin-bottom:16px">${subtitle}</div>

            <div style="width:100%;height:40px;background:#f0f0f0;border-radius:6px;overflow:hidden;position:relative;margin-bottom:12px">
              <div style="height:100%;background:${color};width:${pct}%;display:flex;align-items:center;justify-content:flex-end;padding-right:12px">
                <span style="font-size:18px;font-weight:800;color:#fff">${pct}%</span>
              </div>
            </div>

            ${phaseTopics.length > 0 ? `
              <div style="border-top:1px solid rgba(0,0,0,0.1);padding-top:12px;margin-top:8px">
                <div style="font-size:9px;font-weight:800;letter-spacing:0.06em;text-transform:uppercase;color:rgba(0,0,0,0.4);margin-bottom:8px">TOP TOPICS</div>
                ${phaseTopics.slice(0, 3).map(t => `
                  <div style="font-size:11px;color:${ZD.licorice};margin:5px 0;display:flex;justify-content:space-between;align-items:center;gap:8px">
      ...
      return `
        <div style="display:flex;gap:16px;margin:24px 0 32px;flex-wrap:wrap">
          ${!phase1excluded ? phaseCard(1, phase1, "#C084FC", "#F5F3FF", "Knowledge Retrieval", "Topics automated with Knowledge retrieval") : ""}
          ${phaseCard(2, phase2, "#EC4899", "#FCE7F3", "Automation with Procedures", "Topics automated with procedures and dialogues")}
          ${phaseCard(3, phase3, "#7C3AED", "#EDE9FE", "Automation with Integrations", "Topics automated with integrations / actions")}
        </div>
